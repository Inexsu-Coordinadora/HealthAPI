...
import { ejecutarEnTransaccion } from "../../infraestructura/DBpostgres.js";
import {
    NotFoundError,
    ConflictError,
    BadRequestError,
...
export class CitaMedicaServicio {
    ...
    async CrearCitaMedica(
        ...
        const medicoOcupado =
            await this.citaMedicaRepositorio.verificarCitasSuperpuestasMedico(
                datos.idDisponibilidad,
                datos.fecha
            );
        ...
        const pacienteTieneCita =
            await this.citaMedicaRepositorio.verificarCitasSuperpuestasPaciente(
                datos.idPaciente,
                datos.fecha
            );
        ...
        const consultorioOcupado =
            await this.citaMedicaRepositorio.verificarCitasSuperpuestasConsultorio(
                datos.idDisponibilidad,
                datos.fecha
            );
    ...
    async agendarCitaConValidacion(datos: {
        ...
        if (!pacienteExiste) {
            throw new NotFoundError(
                `Paciente inexistente: El paciente con ID ${datos.idPaciente} no existe en el sistema`
            );
        }
        ...
        console.log(
            `[2] Verificando existencia del Médico ID: ${datos.idMedico}`
        );
        const medicoExiste =
            await this.citaMedicaRepositorio.verificarMedicoExiste(
                datos.idMedico
            );
        if (!medicoExiste) {
            throw new NotFoundError(
                `Médico inexistente: El médico con ID ${datos.idMedico} no existe en el sistema`
            );
        }
        ...
        console.log(
            `[3] Verificando existencia de la Disponibilidad ID: ${datos.idDisponibilidad}`
        );
        ...
        if (idConsultorioFinal) {
            console.log(
                `[4] Verificando existencia del Consultorio ID: ${idConsultorioFinal}`
            );
            ...
            console.log(
                "[4] Consultorio no especificado (se asignará automáticamente)"
            );
        }
        ...
        if (idConsultorioFinal) {
            ...
            console.log(
                "[7] Validación de traslape del Consultorio omitida (no especificado)"
            );
        }

        // VALIDACIÓN 8: Crear la cita con estado "programada" usando TRANSACCIÓN
        console.log(
            "[8] Registrando cita con estado 'programada' (con transacción)"
        );

        const citaRegistrada = await ejecutarEnTransaccion(async (client) => {
            // Todas estas operaciones son ATÓMICAS - o todas se completan o todas se revierten
            ...
            const queryCrearCita = `
                INSERT INTO cita_medica (id_paciente, id_disponibilidad, fecha, estado, motivo, observaciones)
                VALUES ($1, $2, $3, $4, $5, $6)
                RETURNING id_cita, id_paciente, id_disponibilidad, fecha, estado, motivo, observaciones
            `;

            const resultCita = await client.query(queryCrearCita, [
                nuevaCita.idPaciente,
                nuevaCita.idDisponibilidad,
                nuevaCita.fecha,
                nuevaCita.estado,
                nuevaCita.motivo,
            ...
            const cita = resultCita.rows[0];

            // Aquí podrías agregar lógica adicional como actualizar disponibilidad
            // Por ejemplo, marcar la disponibilidad como ocupada si es necesario
            // const queryActualizarDisp = `UPDATE disponibilidad SET ocupada = true WHERE id_disponibilidad = $1`;
            // await client.query(queryActualizarDisp, [datos.idDisponibilidad]);

            console.log(
                `✓ Cita registrada exitosamente con ID: ${cita.id_cita} (transacción confirmada)`
            );
    ...
    async obtenerCitasPorPaciente(
        ...
        if (!paciente) {
            throw new NotFoundError(
                `No se encontró un paciente con el ID ${idPaciente}`
            );
        }

...
export class DisponibilidadRepositorioPostgres implements IDisponibilidadRepositorio {
    ...
    async obtenerDisponibilidadPorId(idDisponibilidad: number): Promise<IDisponibilidad | null> {
        const query = "SELECT * FROM disponibilidad WHERE id_disponibilidad = $1";
    ...
    async listarDisponibilidades(): Promise<IDisponibilidad[]> {
        const query = "SELECT * FROM disponibilidad ORDER BY id_disponibilidad ASC";
    ...
    async obtenerDisponibilidadesPorMedico(idMedico: number): Promise<IDisponibilidad[]> {
        const query = "SELECT * FROM disponibilidad WHERE id_medico = $1 ORDER BY dia_semana, hora_inicio";
    ...
    async obtenerDisponibilidadesPorConsultorio(idConsultorio: number): Promise<IDisponibilidad[]> {
        const query = "SELECT * FROM disponibilidad WHERE id_consultorio = $1 ORDER BY dia_semana, hora_inicio";
    ...
    async actualizarDisponibilidad(
        ...
        const { idDisponibilidad: _, ...datosParaActualizar } = datosDisponibilidad as IDisponibilidad;
        ...
        const columnas = Object.keys(datosParaActualizar)
            .filter((key) => datosParaActualizar[key as keyof typeof datosParaActualizar] !== undefined)
            .map((key) => this.mapearCampoAColumna(key));

        const parametros: Array<string | number | null> = Object.entries(datosParaActualizar)
    ...
    async eliminarDisponibilidad(idDisponibilidad: number): Promise<boolean> {
        const query = "DELETE FROM disponibilidad WHERE id_disponibilidad = $1 RETURNING id_disponibilidad";
        const result = await ejecutarConsulta(query, [idDisponibilidad]);
    ...
    async verificarDisponibilidadDuplicada(
        ...
        if (idConsultorio === null) {
            ...
            query = `
                ...
                AND hora_fin = $5
            `;
            parametros = [idMedico, idConsultorio, diaSemana, horaInicio, horaFin];
        }

...
type TipoEntidad = 'paciente' | 'medico' | 'consultorio';
...
export class CitaMedicaRepositorioPostgres implements ICitaMedicaRepositorio {
    ...
    async crear(datosCita: Omit<ICitaMedica, "idCita">): Promise<ICitaMedica> {
        ...
        const parametros: (string | number | Date | null)[] = Object.values(datosParaInsertar);
    ...
    async obtenerCitaPorId(idCita: number): Promise<ICitaMedica | null> {
        const query = `
            SELECT id_cita, id_paciente, id_disponibilidad, fecha, estado, motivo, observaciones 
            FROM cita_medica WHERE id_cita = $1
        `;
        const result = await ejecutarConsulta(query, [idCita]);
        return result.rows.length === 0 ? null : this.mapearFilaACitaMedica(result.rows[0]);
    }
    ...
    async listarCitas(): Promise<ICitaMedica[]> {
        const query = `
            SELECT id_cita, id_paciente, id_disponibilidad, fecha, estado, motivo, observaciones 
            FROM cita_medica ORDER BY fecha DESC
        `;
    ...
    async actualizarCita(
        ...
        const parametros: (string | number | Date | null)[] = Object.values(datosCita);
    ...
    async obtenerPorPaciente(idPaciente: number): Promise<ICitaMedica[]> {
        const query = `
            SELECT id_cita, id_paciente, id_disponibilidad, fecha, estado, motivo, observaciones 
            FROM cita_medica WHERE id_paciente = $1 ORDER BY fecha DESC
        `;
    ...
    async obtenerPorEstado(estado: string): Promise<ICitaMedica[]> {
        const query = `
            SELECT id_cita, id_paciente, id_disponibilidad, fecha, estado, motivo, observaciones 
            FROM cita_medica WHERE estado = $1 ORDER BY fecha DESC
        `;
    ...
    // 13. Verificar traslapes
    async verificarTraslape(
        tipo: TipoEntidad,
        idEntidad: number,
        horaInicio: string,
        horaFin: string,
        ...
        const condiciones: Record<TipoEntidad, string> = {
            paciente: 'cm.id_paciente = $1',
            medico: 'd.id_medico = $1',
            consultorio: 'd.id_consultorio = $1'
        };

        let query = `
            SELECT cm.id_cita, cm.id_paciente, cm.id_disponibilidad, 
                   cm.fecha, cm.estado, cm.motivo, cm.observaciones 
            FROM cita_medica cm
            INNER JOIN disponibilidad d ON cm.id_disponibilidad = d.id_disponibilidad
            WHERE ${condiciones[tipo]}
            AND cm.estado != 'cancelada'
            AND DATE(cm.fecha) = DATE($4)
            AND (d.hora_fin > $2 AND d.hora_inicio < $3)
        `;
        
        const params: (number | string | Date)[] = [idEntidad, horaInicio, horaFin, fecha];

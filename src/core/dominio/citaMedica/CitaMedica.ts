...
export class CitaMedica implements ICitaMedica {
  ...
  static crear(
    ...
    static desdeBD(idCita:number,idPaciente: number, idDisponibilidad: number, fecha: Date,estado:string, motivo: string, observaciones: string):
        CitaMedica{
            
            return new CitaMedica({idCita, idPaciente, idDisponibilidad, fecha,estado ,motivo, observaciones: observaciones})

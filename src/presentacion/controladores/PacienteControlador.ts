...
import { esquemaCrearPaciente, esquemaPacientePorId, esquemaActualizarPaciente } from "../esquemas/PacienteEsquemas.js";
...
export class PacienteControlador {
    ...
    async crearPaciente(request: FastifyRequest, reply: FastifyReply) {
        const paciente = validadorEsquemas(esquemaCrearPaciente, request.body, reply);
    ...
    async actualizarPaciente(request: FastifyRequest, reply: FastifyReply) {
        const { id: idPaciente } = validadorEsquemas(esquemaPacientePorId, request.params, reply);
        const datos = validadorEsquemas(esquemaActualizarPaciente, request.body, reply);
        ...
        const mensaje = pacienteActualizado ? Mensajes["200_PUT_OK"] : `${Mensajes["404_NOT_FOUND"]} ${idPaciente}`;
        return reply.status(statusCode).send({
            mensaje,
    ...
    async eliminarPaciente(request: FastifyRequest, reply: FastifyReply) {
        ...
        const mensaje = eliminado ? Mensajes["200_DELETE_OK"] : `${Mensajes["404_NOT_FOUND"]} ${idPaciente}`;
